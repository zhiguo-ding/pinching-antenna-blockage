...
for mi = 1 : length(Dlvec) 
    ...
    for i = 1 : ct    
        ...
        blockage_probability = exp(-blockage*dist.^2); % the larger the distance is, the smaller this probability is
        ...
        blockage_probability2 = exp(-blockage*dist2.^2); % the smaller the distance is, the larger this probability is
    ...
    for iy = 1 : length(yvec)
        y = yvec(iy);
        term1 = term1 + (1-exp(-blockage* (y^2+height^2)))*stepsize/D;
    end
    term2 = 0; 
    ...
    for iy = 1 : length(yvec)
        y = yvec(iy);
        term2 = term2 + exp(-blockage* (y^2+height^2))*stepsize/D;
    end
    term3 = 0; 
    ...
    for iy = 1 : length(yvec)
        y = yvec(iy);
        term3 = term3 + exp(-blockage*(y^2+height^2))*stepsize/D;
    end
    ...
    %closed form expressions based on erf
    Pananew(mi) = 1 - exp(-blockage*height^2)*sqrt(pi)/2/sqrt(blockage)/D*...
        (erf(-sqrt(blockage)*tau2) + erf(sqrt(blockage)*tau3));
    Pappnew(mi) = 1 - sqrt(pi)*exp(-blockage*height^2)/sqrt(blockage)/D*...
        erf(sqrt(blockage)*D/2);
...
semilogy(Dlvec, po_conv_block, Dlvec, po_pin_block, Dlvec, Pana,Dlvec, Pananew)
